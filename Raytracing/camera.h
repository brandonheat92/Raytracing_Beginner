...
class camera
{
    public:
        ...
        void render(const hittable& world)
        {
            ...
            for (int indx = 0; indx < image_height; indx++)
            {
                std::clog << "\rScanlines remaining: " << (image_height - indx) << ' ' << std::flush;
                for (int indx1 = 0; indx1 < image_width; indx1++)
                {
                    color pixel_color(0, 0, 0);
                    for (int sample = 0; sample < samples_per_pixel; sample++)
                    {
                        ray r = get_ray(indx1, indx);
                        pixel_color += ray_color(r, world);
                    }
                    vec3 colorV;
                    write_color(std::cout, pixel_samples_scale * pixel_color, colorV);

                    int index = (indx * image_width + indx1) * 3;
